...
import com.daml.metrics.{Metrics, Timed, SpanKind}
...
private[daml] class JdbcIndexer private[indexer] (
  ...
  private def handleStateUpdate(
    ...
    Flow[(Offset, UpdateWithContext)]
      ...
      .mapAsync(1){ t => 
        t._2.telemetryContext.runFutureInNewSpan("prepareTransationInsert", SpanKind.Internal){ telemetryContext =>
          prepareTransactionInsert(t._1, t._2.update).map(ou => OffsetUpdateWithContext(ou)(telemetryContext))(mat.executionContext)
        }
      }
      .mapAsync(1) { ouwc =>
        withEnrichedLoggingContext(JdbcIndexer.loggingContextFor(ouwc.offsetUpdate.offset, ouwc.offsetUpdate.update)) {
          implicit loggingContext =>
            ouwc.telemetryContext.runFutureInNewSpan("executeUpdate", SpanKind.Internal) {_ =>
              Timed.future(
                metrics.daml.indexer.stateUpdateProcessing,
                executeUpdate(ouwc.offsetUpdate),
              )
            }
        }
      }
