...
private[dao] final class TransactionsReader(
      ...
      .wireTap(_ match {
        case (_, response) =>
          response.transactions.foreach(txn =>
            Spans.addEventToSpan(daml.metrics.Event("transaction", TraceIdentifiers.fromTransaction(txn)), span)
          )
      })
  ...
  def getTransactionTrees(
    ...
    groupContiguous(events)(by = _.transactionId)
      ...
      .wireTap(_ match {
        case (_, response) =>
          response.transactions.foreach(txn =>
            Spans.addEventToSpan(daml.metrics.Event("transaction", TraceIdentifiers.fromTransactionTree(txn)), span)
          )
      })
      .watchTermination()(endSpanOnTermination(span))
  }
  ...
  def getActiveContracts(
    ...
    groupContiguous(events)(by = _.transactionId)
      ...
      .buffer(outputStreamBufferSize, OverflowStrategy.backpressure)
      .wireTap(response => {
        Spans.addEventToSpan(com.daml.metrics.Event("contract", Map((SpanAttribute.Offset, response.offset))), span)
      })
      .watchTermination()(endSpanOnTermination(span))
  }
